...
- [jumpstart design](#jumpstart-design)
...
`jumpstart` is designed to allow developers to instantly create a project for local scripts written in Typescript.

## How is it different from what I already use?

**WORK IN PROGRESS**: `jumpstart` is still in the `design` phase.
...
`jumpstart` is a part of the [Open Source Universe](https://github.com/intellibus/approach). All Projects in the [Open Source Universe](https://github.com/intellibus/approach) follow the same [Contribution Guidelines & Process](https://github.com/intellibus/approach).
...
`jumpstart` only plans to initially support Typescript & JavaScript Applications. However, we would love for the community to contribute versions for other languages (Java, Go, etc.)
